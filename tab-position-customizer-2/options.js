...
function initText() {
	var i18nTexts = [
			...
			"open", "openFirst", "openLast", "openRight",
			"openLeft", "openDefault",
			
			"close", "closeFirst", "closeLast", "closeRight",
			"closeLeft", "closeOrder", "closeDefault",
			
			"new", "newForeground", "newBackground", "newDefault",
			
			"misc", "miscSameWindow", "miscSameWindowException",
			"miscExternalLinkDefault", "miscExternalLinkUnfocus"
		];
	
	for (var i = 0; i < i18nTexts.length; i++) {
		document.getElementById(i18nTexts[i] + "Text").innerHTML
				 = chrome.i18n.getMessage(i18nTexts[i]);
	}
}
...
function restoreSameWindowForm() {
	...
	sameWindowForm.exception.addEventListener('input', saveSameWindowFormException, false);
	sameWindowForm.addEventListener('reset',
		function(event) {
...
function restoreFormButton(formName, storageKey) {
	...
	for (var i = 0; i < form.length; i++) {
		...
		form[i].addEventListener('click', function() {saveFormButton(formName, storageKey)}, false);
	}
	if (checked == false) {
...
function saveSameWindowForm() {
	var sameWindowForm = document.same_window;
	localStorage["AlwaysSameWindow"] = sameWindowForm.true.checked ? "true" : "false";
...
function saveExternalLinkDefault() {
	var externalLinkDefault = document.external_link_default;
	localStorage["ExternalLinkDefault"] = externalLinkDefault.true.checked ? "true" : "false";
}

function saveExternalLinkUnfocus() {
	var externalLinkUnfocus = document.external_link_unfocus;
	localStorage["ExternalLinkUnfocus"] = externalLinkUnfocus.true.checked ? "true" : "false";
}
